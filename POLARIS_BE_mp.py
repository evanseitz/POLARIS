...
def init():
    ...
    #    v.1.0 Python 3.x                                                    #\n\
    ##########################################################################\n\
    #    Copyright (C) 2018 Evan Elliott Seitz                               #\n\
    ##########################################################################\n\
    #                                                                        #\n\
    #                               LICENSE                                  #\n\
    #                                                                        #\n\
...
# find all line permutations for a given order:
    ...
    def line_permute(start, end, blocks_min, N, R):
        ...
        if (len(blocks_min) - R) < 0:
            ...
            if degen > 0: #more than one equal-energy minima
                print('#  Degeneracy:', degen)
                distances = [] #take coordinates from minima with overall shortest euclidean distance
                dist_idxs = []
                for idx in min_idxs[0]: #e.g., idx = [781 822 909]
                    dist_idxs.append(idx)
                    dist = 0
                    pts = []
                    for p in tempLines[idx]:
                        pts.append(p)
                ...
                short_idx_loc = np.argmin(distances)
                short_idx_glob = dist_idxs[short_idx_loc]
                minLine = tempLines[short_idx_glob]
                minLineEnergy = lineErgs[short_idx_glob]
...
# call functions and plot:
    ...
    np.savetxt(outname_DF,
               ...
               fmt='%i %i %10.4f',
               header=('x  y \t  energy'),
               comments=('POLARIS output\n\ninput points: %s\ninput settings: N: %s, R: %s\npath energy: %s\npath length: %s\nelapsed time: %s min\n\n'\
                         % (user_pts_final,user_N,user_R,final_e_all,len(np.vstack(final_e)),timer)))
